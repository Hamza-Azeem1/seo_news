...
const CommentSection = ({ blogId, userId }) => {
    ...
    const fetchComments = useCallback(async () => {
        try {
            const response = await axios.get(`http://localhost:8000/api/comments/${blogId}/comments`);
    ...
    const handleSubmitComment = async () => {
        if (comment.trim() !== '') {
            try {
                await axios.post(`http://localhost:8000/api/comments/${blogId}/comments`, { text: comment });
                fetchComments();
                setComment('');
            } catch (error) {
    ...
    const handleLike = async (index) => {
        ...
        try {
            await axios.post(`http://localhost:8000/api/comments/${comment._id}/like`, { userId });
            if (comment.liked) {
                comment.likes -= 1;
                comment.liked = false;
            } else {
    ...
    const handleReplySubmit = async (index) => {
        ...
        if (comment.replyText.trim() !== '') {
            try {
                await axios.post(`http://localhost:8000/api/comments/${comment._id}/replies`, { text: comment.replyText });
